  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `https://finnapi.sellingpartnerservice.com/ReportAgent?Query=${encodeURIComponent(
          currentQuery
        )}`,
        {
          method: "GET",
      ...
      // Always keep only one message (no history)
      setMessages([{ query: currentQuery, result: resultData }]);
    } catch (err) {
    ...
    <DashboardLayout>
      <div className="finngpt-wrapper">
        ...
        <div className="finngpt-main">
          {/* FIXED: Show initial screen only when no messages */}
          {messages.length === 0 ? (
            ...
            <>
              ...
              <div className="finngpt-input-container">
                <div className="finngpt-input-wrapper chatgpt-style">
                  ...
                  {loading && (
                    <div className="finngpt-analysing">Analysing...</div>
                  )}
                  <Form.Control
                    ref={textareaRef}
                    as="textarea"
